module.exports = {
  ...
  subtitle (data) {
    ...
    if (info === 0) {
      return data.color ? `Create with color "${data.color}"` : 'No background color has been created'
    } if (info === 1) {
      return data.gradient ? `Create with gradient "${data.gradient}"` : 'No gradient background created'
    }
  },
...
<div>
  ...
  <span class="dbminputlabel">To fill in</span>
