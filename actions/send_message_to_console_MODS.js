module.exports = {
  ...
  if (data.retornar == 1 && data.varName.length > 0) {
    return `See object of ${storage[parseInt(data.storage, 10)]} (${data.varName})`
  }
  return 'Put a variable'
},
  ...
  html (isEvent, data) {
    ...
    <div style="width:100%;">
        <span class="dbminputlabel">Return</span>
        <select id="retornar" class="round" onchange="glob.onChange1(this)">
        <option value="0" selected>Text with color in console</option>
        <option value="1">Show variable object information</option>
        </select>
    </div>
  <br>
  ...
  <span class="dbminputlabel">Object Variable</span><br>
...
<span class="dbminputlabel">Message to send</span><br>
  ...
  action (cache) {
    ...
    if (send.length > 0) {
      ...
      console.log(chalk.gray(`Provide something to log: Action #${cache.index + 1}`))
      this.callNextAction(cache)
    }}
